...
///
/// ```
/// use siderust_pod_io::vgosdb::VgosDb;
/// // VgosDb cannot be constructed until the feature is implemented.
/// let err = siderust_pod_io::vgosdb::read_vgosdb(std::path::Path::new("/none")).unwrap_err();
/// assert!(matches!(err, siderust_pod_io::PodIoError::Unsupported(_)));
/// ```
#[derive(Debug)]
pub struct VgosDb;
